...
import { processScheduledOutboundDrafts } from "@/lib/email-inbox/server";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await processScheduledOutboundDrafts();
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to process scheduled outbound drafts",
      },
      { status: 500 },
    );
  }
