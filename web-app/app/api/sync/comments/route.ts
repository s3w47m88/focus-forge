...
import { sendTaskCommentNotifications } from "@/lib/task-notifications";
...
const canAccessTaskOrProject = async (
  ...
  if (task.project_id && visibleProjectIds.has(String(task.project_id)))
    return true;
  if (task.assigned_to && visibleUserIds.includes(String(task.assigned_to)))
    return true;
...
export async function GET(request: NextRequest) {
  ...
  const enrichedComments = await enrichCommentsWithAuthors(
  ...
  try {
    ...
    if (!content || (!taskId && !projectId)) {
      return createErrorResponse(
        "Content and either taskId or projectId are required",
        400,
      );
    }
    ...
    if (taskId) {
      void sendTaskCommentNotifications({
        taskId,
        actorUserId: userId,
    ...
    const [enrichedComment] = await enrichCommentsWithAuthors([
