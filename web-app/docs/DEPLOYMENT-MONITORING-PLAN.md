...
railway up -s "Focus: Forge" --detach
...
if [ $? -ne 0 ]; then
    ...
    railway variables -s "Focus: Forge" > deployment-vars.txt
    railway logs -s "Focus: Forge" > deployment-logs.txt
    
    # Check for common issues
    if grep -q "Supabase" deployment-logs.txt; then
        echo "⚠️ Supabase configuration issue detected"
        echo "Current variables:"
        railway variables -s "Focus: Forge" | grep SUPABASE
    fi
fi
```
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to Railway
        ...
        run: |
          railway up --service "Focus: Forge" --detach
...
for var in "${REQUIRED_VARS[@]}"; do
  value=$(railway variables -s "Focus: Forge" | grep "^$var=" | cut -d'=' -f2)
  if [ -z "$value" ]; then
